// Replace the populateWeakTopicsTable function in teacher-dashboard.html with this:
...
function populateWeakTopicsTable(students) {
    ...
    students.forEach(student => {
        ...
        Object.keys(student.topicPerformance).forEach(topic => {
            ...
            if (perf.attempted >= 3) {
                ...
                if (successRate < 60 || perf.failureStreak >= 2) {
                    weakTopics.push(DATA.topics[topic]);
                }
            }
        });

        // Calculate overall score (success rate)
        const score = student.codeSuccessRate;
        ...
        if (score >= 85 && student.totalQuestions >= 30 && weakTopics.length === 0) {
            status = 'Master';
            statusBadge = 'badge-success';
        } else if (score >= 70 && student.totalQuestions >= 20) {
            status = 'Advanced';
            statusBadge = 'badge-primary';
        } else if (score >= 50 && student.totalQuestions >= 10) {
            status = 'Intermediate';
            ...
            <tr>
                <td>
                    <div style="display: flex; flex-direction: column;">
                        ...
                        <span style="font-size: 0.75rem; color: var(--text-muted);">${student.id}</span>
                    </div>
                </td>
                ...
                <td>
                    <div style="display: flex; flex-direction: column; gap: 0.25rem;">
                        ...
                        <span style="font-size: 0.75rem; color: var(--text-muted);">${student.totalQuestions} questions</span>
                    </div>
                </td>
                <td>
                    <div style="display: flex; flex-direction: column; gap: 0.25rem;">
